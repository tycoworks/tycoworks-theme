...
```bash
npm install                       # also generates the agent skill
...
theme.json                     the map onto it, and the only file to author
assets/                        icons, brand marks, backdrops
showcase.md                    a deck exercising every layout
```

`SKILL.md`, `syntax.md`, `manifest.json` and `assets.json` are generated by
`npx tycoslide package` and are not checked in. Layout names, parameters and
authoring syntax live there.
